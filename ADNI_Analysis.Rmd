...
df_subset <- na.omit(df[, c('DX','Hippocampus', 'AGE', 'PTGENDER','VSWEIGHT', 'VSHEIGHT', 'PTEDUCAT', 'Partner', 'CDRSB', 'ADAS13', 'MMSE', 'RAVLT_perc_forgetting_bl', 'FAQ', 'DIGITSCOR')])
df_subset <- df_subset[df_subset$RAVLT_perc_forgetting_bl >=0,]
#mean center data
df_subset$FAQ <- log(df_subset$FAQ + 0.001)
...
binnedplot(y=df_subset$DX == 'Dementia',df_subset$AGE,xlab="AGE",ylim=c(0,1),col.pts="navy",
...
Model4 <- lm(Hippocampus ~DIGITSCOR + FAQ +RAVLT_perc_forgetting_bl + VSWEIGHT +AGE + PTGENDER , data=df_subset)
summary(Model4)
```
```{r}
